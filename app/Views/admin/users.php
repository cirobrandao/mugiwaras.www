								...
								<div class="row g-2">
									...
									<?php if (\App\Core\Auth::isSuperadmin($currentUser)): ?>
										<div class="col-12 mt-3">
											<hr class="my-2">
											<form method="post" action="<?= base_path('/admin/users/update-expires') ?>">
												<input type="hidden" name="_csrf" value="<?= View::e($csrf) ?>">
												<input type="hidden" name="id" value="<?= (int)$u['id'] ?>">
												<label class="form-label">Assinatura expira em</label>
												<input class="form-control" type="datetime-local" name="subscription_expires_at" value="<?= View::e(!empty($u['subscription_expires_at']) ? date('Y-m-d\TH:i', strtotime($u['subscription_expires_at'])) : '') ?>">
												<div class="form-text">Formato local. Deixe vazio para remover vencimento.</div>
												<div class="mt-2">
													<button class="btn btn-sm btn-outline-primary" type="submit">Atualizar vencimento</button>
												</div>
											</form>
										</div>
									<?php endif; ?>
