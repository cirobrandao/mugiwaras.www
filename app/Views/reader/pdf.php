...
<?php if (empty($error) && !empty($content)): ?>
    ...
    <?php else: ?>
        ...
        <div id="readerWrap" class="reader-shell">
            <div class="reader-toolbar mb-2">
                <div class="toolbar-row">
                    ...
                    <div class="reader-toolbar-right ms-auto">
                        ...
                        <?php if (!empty($downloadUrl)): ?>
                            <a class="btn btn-sm btn-outline-primary" href="<?= $downloadUrl ?>" title="Download PDF" aria-label="Download PDF"><i class="fa-solid fa-download"></i></a>
                        <?php endif; ?>
            ...
            <div id="reader" class="reader-frame" data-total="<?= count($pages ?? []) ?>" data-base-url="<?= base_path('/reader/pdf/' . (int)($content['id'] ?? 0) . '/page') ?>" data-content-id="<?= (int)($content['id'] ?? 0) ?>" data-csrf="<?= View::e($csrf ?? '') ?>" data-last-page="<?= (int)($lastPage ?? 0) ?>" data-direction="rtl">
