...
$fullLink = url('support/track/' . (string)$token);
?>
<h1 class="h4 mb-3">Chamado #<?= str_pad((string)(int)$ticket['id'], 4, '0', STR_PAD_LEFT) ?> - <?= View::e((string)$ticket['subject']) ?></h1>
<?php if (!empty($_GET['sent'])): ?>
    <div class="alert alert-success">Chamado criado com sucesso. Guarde este link para acompanhar.</div>
<?php endif; ?>
<div class="alert alert-info">
    <div class="mb-2">Link de acompanhamento:</div>
    <div class="input-group">
        <input type="text" class="form-control" value="<?= View::e($fullLink) ?>" readonly data-support-track-link>
        <button class="btn btn-outline-secondary" type="button" data-support-track-copy>Copiar</button>
    </div>
    <div class="small text-muted mt-2">Guarde este link para acompanhar o atendimento.</div>
</div>
<div class="mb-3">
...
<div class="card">
    <div class="card-body">
        ...
        <?php if (!empty($canReply)): ?>
            ...
            <form method="post" action="<?= base_path('/support/track/' . (string)$token . '/reply') ?>" enctype="multipart/form-data">
